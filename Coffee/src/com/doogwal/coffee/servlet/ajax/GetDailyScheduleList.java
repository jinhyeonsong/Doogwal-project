...
@WebServlet("/ajax/getDailyScheduleList.json")
public class GetDailyScheduleList extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		String dateAll = req.getParameter("dateAll");
		String dateStr = req.getParameter("dateStr");//sql에서 해당 날짜 스케줄 불러오기 위해 필요함.
		...
		//Meeting dailySchedule = new Meeting(crewNo,startDate);//크루번호랑 시작 날짜 넣고
		//시작날짜에 해당하는 스케줄 번호 받아오기 위함 ====> 이거 대신에 HashMap 사용함
		
		List<Meeting> dailySchedules = SchedulesDAO.selectDailySchedules(map);
		...
		for(Meeting dailySchedule: dailySchedules) {
			CrewCalendar crewCalendar = new CrewCalendar();
			
			scheduleNo = dailySchedule.getNo();//스케줄 번호 얻기
			
			type = dailySchedule.getType();//스케줄 타입 얻기 G,F,M 이렇게 있음.
			
			name= dailySchedule.getName();//일정 이름 얻기 
			startTime = dailySchedule.getStartDate();//일정 시작 시간 얻기
